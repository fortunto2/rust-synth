...
pub(super) const AUTO_EVOLVE_STRENGTH: f32 = 0.55;
...
fn advance_beat_sync(app: &mut AppState, engine: &EngineHandle) {
    ...
    for _ in 0..steps {
        if app.coupling {
            super::evolve::seed_from_audio(app, engine, cur_beat);
        }
        app.life.step();
    }
    ...
    if app.auto_evolve && cur_beat - app.last_evolve_beat >= app.evolve_period as i64 {
        if let Some((name, before, after)) = super::evolve::evolve_weakest(app, engine) {
...
fn handle_key(key: KeyEvent, engine: &EngineHandle, app: &mut AppState) {
    ...
    match key.code {
        ...
        KeyCode::Char('e') => {
            super::evolve::mutate_selected(app, engine, 0.3);
            return;
        }
        KeyCode::Char('E') => {
            super::evolve::mutate_all_active(app, engine, 0.25);
            return;
        }
        KeyCode::Char('x') => {
            super::evolve::crossover_with_neighbor(app, engine);
            return;
        }
        KeyCode::Char('R') => {
...
fn handle_tracks_key(key: KeyEvent, engine: &EngineHandle, app: &mut AppState) {
    ...
    match key.code {
        ...
        KeyCode::Char('r') => {
            let p = &tracks[app.selected_track].params;
            super::evolve::randomize_track(p, &mut app.rng_seed);
        }
        _ => {}
    }
}
...
fn handle_params_key(key: KeyEvent, engine: &EngineHandle, app: &mut AppState) {
    ...
    match key.code {
        ...
        KeyCode::Char('r') => super::evolve::randomize_track(&track.params, &mut app.rng_seed),
        _ => {}
    }
}
